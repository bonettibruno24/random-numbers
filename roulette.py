...
while True:
    ...
    class GameAnalytics:
        ...
        def add_to_sequence(self, number):
            if not number:  # Verifica se o número é None ou vazio
                print("Erro: Nenhum valor foi inserido.")
                return
            try:
                self.__sequence__numbers__.append(int(number))
                print(f"Número {number} adicionado à sequência: {self.__sequence__numbers__}")
            except ValueError:
                print("Erro: o valor inserido não é válido.")
    game = GameAnalytics()

    # Caminho completo para o arquivo final.xlsx
    ...
    if number2 is not None:  # Verifica se number2 não é None antes de usar
        ...
        later_position = None
        position_array_index = None
        algarism_previus = None
        algarism_later = None
    ...
    def veirify_algarism():
        ...
        if  (last_digit_insert == last_digit_insert2) or (last_digit_insert1 == last_digit_insert2):
            print(f"{numero}, {number1}, {number2}", "OK")
            return True 
        elif (last_digit_insert == algarism_later) or (last_digit_insert == algarism_previus):
            print(f"{numero}, {number1} ,{number2} OK")
            return True
        elif (last_digit_insert1 == algarism_later) or (last_digit_insert1 == algarism_previus):
            print(f"{numero}, {number1} ,{number2} OK")
            return True
    ...
    """
    Prob1: Verifica quantas vezes o ultimo digito do numero && number2 está presente na lista dos ultimas jogadas e divide por 50.
    ...
    probability = calculate_probability(game.df_most_frequent, last_digit_insert, last_digit_insert1)
    if probability != "0.00%":
    ...
    if getattr(sys, 'frozen', False):
        # Diretório temporário onde os arquivos são extraídos pelo PyInstaller
        base_dir = sys._MEIPASS
    else:
        # Diretório do script quando rodando como .py
        base_dir = os.path.dirname(os.path.abspath(__file__))

    # Caminho do arquivo extraído pelo PyInstaller
    source_path = os.path.join(base_dir, 'final.xlsx')

    # Caminho para onde o arquivo deve ser copiado no diretório do executável
    destination_path = os.path.join(os.getcwd(), 'final.xlsx')

    # Se o arquivo ainda não existe, copia da pasta temporária para a pasta do executável
    if not os.path.exists(destination_path):
        shutil.copy(source_path, destination_path)

    # Agora, use destination_path para abrir com o openpyxl
    file_path = destination_path

    if os.path.exists(file_path):
        # Se o arquivo existir, lê o conteúdo existente
        ...
        print(f"Dados inseridos com sucesso no arquivo existente: {file_path}")
    else:
        # Se o arquivo não existir, cria um novo arquivo com os dados
        df.to_excel(file_path, index=False)
        print(f"Arquivo criado e dados inseridos: {file_path}")
